...
# from myproject.forms import LoginForm, ProductForm, RegistrationForm,ApplyForm, VolunteerForm
...
# @app.route('/h_t')
# def h_t():
#     return render_template('h_t.html')
...
#         try:
#             user = User(username=form.username.data, password=form.password.data,user_id= form.user_id.data,role =form.role.data)
#             # add to db table
#             db.session.add(user)
#             db.session.commit()
...
#         return render_template('products.html',infos = infos)
#     elif request.method == "POST":
#         Product.query.filter(Product.title.ilike(f'%{keyword}%'))
...
#             flash("物件編號不存在")
#             return render_template('send.html')
#         k = Product.query.filter_by(id = int(request.form.get('case_num'))).first()
#         if k.uploader_id != current_user.user_id:
#             flash("此物件非您擁有")
#             return render_template('send.html')
#         email = MIMEMultipart()
#         email["subject"] = f"需求申請回覆 FROM {current_user.user_id}：物件號碼{request.form.get('case_num')}"  #郵件標題
#         email["from"] = 'linjay017@gmail.com'  #寄件者
#         email["to"] = request.form.get('reciever')
#         inner = request.form.get('content')
#         email.attach(MIMEText(inner))  
#         with smtplib.SMTP(host="smtp.gmail.com", port="587") as smtp:  # 設定SMTP伺服器
#             try:
#                 smtp.ehlo()  # 驗證SMTP伺服器
#                 smtp.starttls()  # 建立加密傳輸
#                 smtp.login("linjay017@gmail.com", "ldxjztbmrtuuvtmn")  # 登入寄件者gmail
...
#     elif request.method == "POST":
#         email = MIMEMultipart()
#         email["subject"] = f"需求申請 申請機構： {current_user.user_id}：物件號碼 {array_num}"  #郵件標題
#         email["from"] = 'linjay017@gmail.com'  #寄件者
#         email["to"] = request.form.get('reciever')
#         inner = request.form.get('content')+f'\r\n--------------------------------------------------------\r\n備註：您可以選擇直接與機構透過電子郵件聯絡。若您不願意透露電子郵件，也可以透過 http://127.0.0.1:8000/send 進行回覆。請勿回覆本郵件！\r\n申請機構電子郵件：{current_user.username}'
#         email.attach(MIMEText(inner))  
#         with smtplib.SMTP(host="smtp.gmail.com", port="587") as smtp:  # 設定SMTP伺服器
#             try:
#                 smtp.ehlo()  # 驗證SMTP伺服器
